...
/**
 * The action to be used to add a player to the white list.
 * 
 * @author vincent
 */
public class WhitelistAction extends AbstractAction {
	 ...
	 */
	private static final ResourceBundle MESSAGES = new Messages(WhitelistAction.class);

	private static final String MSG_CLASS_NAME = WhitelistAction.class.getSimpleName();
	 ...
	 *            The list with the selected items.
	 */
	public WhitelistAction(final Globals globals, final JList<String> list) {
		super();
	...
	public void actionPerformed(final ActionEvent event) {
		...
		try {
			for (final String player : selectedPlayers) {
				if (connection != null) {
					connection.whitelistAdd(player);
				}
			}
